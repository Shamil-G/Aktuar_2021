...
def rep_unemployment_0703(id_calc):
	now = datetime.datetime.now()

	file_name = 'unemployment_0703_' + str(id_calc) + '.xlsx'
	...
	if os.path.isfile(file_path):
		...
		cursor.execute(
			...
			"and substr(al.rfpm_id,1,4)='0703'", [id_calc])
		...
		print("Завершен расчет по безработным : "+now.strftime("%d-%m-%Y %H:%M:%S"))
